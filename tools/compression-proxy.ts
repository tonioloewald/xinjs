...
console.log(
  `baseline dist/module.js  gz ${gz(enc.encode(base))}  br ${br(
    enc.encode(base)
  )}`
)
console.log(
...
const row = (name: string, mutated: string): [number, number] => {
  ...
  console.log(
    `${name.padEnd(46)} ${String(dGz).padStart(7)} ${String(dBr).padStart(
      7
    )}   ${ratio.toFixed(2)}`
  )
  return [dGz, dBr]
...
rows.push(
  row(
    'add a NEW 100-char error string',
    base +
      `\nconst _m = ${JSON.stringify(
        'Refusing: the path names a list item by index, which moves when the list reorders; name it by id instead.'
      )}\n`
  )
)
rows.push(
  row(
    'add 400 chars of incompressible base64',
    base +
      '\n// ' +
      btoa(
        String.fromCharCode(...crypto.getRandomValues(new Uint8Array(300)))
      ) +
      '\n'
  )
)
...
const r = (() => {
  const x = rows.map((p) => p[0]),
    y = rows.map((p) => p[1])
  const mx = mean(x),
    my = mean(y)
  const num = x.reduce((a, _, i) => a + (x[i] - mx) * (y[i] - my), 0)
  const den = Math.sqrt(
    x.reduce((a, v) => a + (v - mx) ** 2, 0) *
