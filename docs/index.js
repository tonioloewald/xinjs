...
`,n=n.substring(m.length+1),S=!0;if(!S){let M=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),D=new RegExp(`^ {0,${Math.min(3,i-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),H=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:\`\`\`|~~~)`),Dn=new RegExp(`^ {0,${Math.min(3,i-1)}}#`);while(n){if(p=n.split(`
`,1)[0],m=p,this.options.pedantic)m=m.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ");if(H.test(m))break;if(Dn.test(m))break;if(M.test(m))break;if(D.test(n))break;if(m.search(/[^ ]/)>=i||!m.trim())u+=`
`+m.slice(i);else{if(l)break;if(h.search(/[^ ]/)>=4)break;if(H.test(h))break;if(Dn.test(h))break;if(D.test(h))break;u+=`
